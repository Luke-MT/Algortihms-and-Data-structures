...
void* upo_ht_sepchain_put(upo_ht_sepchain_t ht, void *key, void *value)
{
    ...
    if(ht != NULL && ht->slots != NULL){
        ...
        upo_ht_sepchain_list_node_t *current = (ht->slots+index)->head;

        while(current != NULL && ht->key_cmp(current->key,key) != 0)
            current = current->next;

        if(current == NULL){
            current = malloc(sizeof(upo_ht_sepchain_list_node_t));
            current->key = key;
            current->value = value;
            current->next = (ht->slots+index)->head;
            (ht->slots+index)->head = current;
            ht->size++;
        }
        else{
...
void upo_ht_sepchain_delete(upo_ht_sepchain_t ht, const void *key, int destroy_data)
{
    ...
    if(ht != NULL && ht->slots != NULL){
        ...
        if(current != NULL){
            if(p == NULL)
                (ht->slots+index)->head = current->next;
            else
                p->next = current->next;
...
size_t upo_ht_sepchain_collisions(const upo_ht_sepchain_t ht, const void* key){
    if(ht != NULL && ht->slots != NULL){
        size_t index = ht->key_hash(key,ht->capacity);
        upo_ht_sepchain_list_node_t *current = ht->slots[index].head;
        size_t size = 0;
...
void* upo_ht_linprob_put(upo_ht_linprob_t ht, void *key, void *value)
{
    ...
    while((ht->slots[index].key != NULL 
        ...
        if(ht->slots[index].tombstone && found_tomb == 0){
            found_tomb = 1;
            index_tomb = index;
        }
...
void* upo_ht_linprob_get(const upo_ht_linprob_t ht, const void *key)
{
    ...
    size_t index = ht->key_hash(key,ht->capacity);
    while((ht->slots[index].key != NULL 
           && ht->key_cmp(key,ht->slots[index].key) != 0)
           || ht->slots[index].tombstone == 1)
        index = (index+1) % ht->capacity;
...
upo_ht_key_list_t upo_ht_sepchain_keys(const upo_ht_sepchain_t ht)
{
    ...
    for(size_t i = 0; i < ht->capacity; i++){
        ...
        for(node = ht->slots[i].head; node !=NULL; node = node->next){
            upo_ht_key_list_node_t *node_list = malloc(sizeof(upo_ht_sepchain_list_node_t));
...
void upo_ht_linprob_traverse(const upo_ht_linprob_t ht, upo_ht_visitor_t visit, void *visit_context)
{
    ...
    if(upo_ht_linprob_is_empty && visit == NULL)
        return;
